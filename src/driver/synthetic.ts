...
interface SyntheticOpts {
  mid: number;
  hz: number; // events/sec
  levels: number; // Ticks out from mid
  tick: number; // Price increment
}

// Creates synthetic orders for testing
// Orders never cross spread
export function startSynthetic(book: OrderBook, opts: SyntheticOpts): () => void{
    const ids:Array<string> = [];
    let n = 0;

    const synInt = setInterval(() => {
        // Self balancing add/cancel ration that pushed toward a target book size of 12*levels
        // When ids is empty pAdd === 1 guarenteeing an order is added and not canceled
